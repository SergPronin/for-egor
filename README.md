...
- `MatrixCalculatorUI.java`: Реализует графический интерфейс с таблицами для ввода матриц, кнопками операций и областью вывода результатов.
...
- `att3.java`: Реализует нахождение обратной матрицы (для матриц 2×2 и произвольного размера) и умножение матриц.
...
2. **Операции**:
   - Нажмите одну из кнопок операций:
     ...
     - **Найти обратную матрицу 2×2**: Для матрицы A размером 2×2.
   ...
   - Результат или сообщение об ошибке отобразится в области вывода.
...
- **Математическая суть**: Определитель — скаляр, связанный с квадратной матрицей, используется для решения систем уравнений и проверки обратимости.
- **Как работает**:
...
- **Пример**:
  - Ввод: `matrix = [[1, 2], [3, 4]]`
  - Вычисление: det = 1 × 4 - 2 × 3 = -2
  - Вывод: `Определитель: -2.0`

#### 2. `computeDeterminant(double[][] matrix)` (приватный)
- **Назначение**: Рекурсивно вычисляет определитель матрицы.
- **Математическая суть**: Для 1×1: возвращает элемент. Для 2×2: a × d - b × c. Для n×n (n > 2): разложение по первой строке с кофакторами.
- **Как работает**:
  1. Если n = 1, возвращает `matrix[0][0]`.
  2. Если n = 2, вычисляет a × d - b × c.
  3. Для n > 2: суммирует `matrix[0][j] × computeCofactor(0, j)`.
- **Код**:
  ```java
...
- **Математическая суть**: Удаляет строку и столбец, создавая матрицу (n-1)×(n-1).
- **Как работает**:
  1. Создаёт матрицу (n-1)×(n-1).
...
- **Назначение**: Вычисляет обратную матрицу 2×2.
- **Математическая суть**: A^(-1) = (1/det) × [[d, -b], [-c, a]].
- **Как работает**:
  1. Проверяет размер 2×2.
...
- **Математическая суть**: Для A (m×n) и B (n×p), результат — m×p.
- **Как работает**:
...
- **Математическая суть**: result[i][j] = Σ (A[i][k] × B[k][j]).
- **Как работает**:
  1. Создаёт матрицу A.length × B[0].length.
