...
Нахождение обратной матрицы 2x2 с проверкой.
Умножение двух квадратных матриц.
Нахождение обратной матрицы произвольного размера.
...
Для 1: Кофактор = (-1)^(1+1) * det([5 6; 8 9]) = 1 * (59 - 68) = 1 * (45 - 48) = -3
Для 2: Кофактор = (-1)^(1+2) * det([4 6; 7 9]) = -1 * (49 - 67) = -1 * (36 - 42) = 6
Для 3: Кофактор = (-1)^(1+3) * det([4 5; 7 8]) = 1 * (48 - 57) = 1 * (32 - 35) = -3
det = 1*(-3) + 26 + 3(-3) = -3 + 12 - 9 = 0
...
Вычислить определитель матрицы A (det(A)).
Если det(A) = 0, решения нет или их бесконечно много.
...
det(A) = 23 - 11 = 6 - 1 = 5
A1 (заменяем 1-й столбец на B):[ 5  1 ]
[ 4  3 ] → det(A1) = 5*3 - 1*4 = 15 - 4 = 11 → x1 = 11/5 = 2.2


A2 (заменяем 2-й столбец на B):[ 2  5 ]
...
Используем элементарные преобразования строк:
Сложение строки с другой, умноженной на константу.
Перестановка строк.


Обнуляем элементы под диагональю, двигаясь слева направо.
Определитель такой матрицы равен произведению диагональных элементов, с учётом знака от перестановок строк.
...
Вычтем из 2-й строки 41-ю, из 3-й — 71-ю:[ 1  2   3  ]
[ 0  -3  -6 ]
[ 0  -6  -12 ]


Вычтем из 3-й строки 2*2-ю:[ 1  2   3  ]
[ 0  -3  -6 ]
[ 0  0   0  ]
...
Одно решение: Ранг матрицы A равен рангу [A|B] и числу неизвестных.
Бесконечное число решений: Ранг меньше числа неизвестных.
Нет решений: Ранг [A|B] больше ранга A.

Шаги:

Составить расширенную матрицу [A|B].
Привести к ступенчатому виду (прямой ход).
Проверить ранги.
Если решение единственное, выполнить обратный ход для нахождения x.
...
Вычтем из 2-й строки 2*1-ю:[ 1  1  |  3 ]
[ 0  0  |  0 ]
...
Составить матрицу [A|I].
Привести левую часть к единичной матрице.
Правая часть станет A^-1.
Если det = 0, обратной матрицы нет.
...
det = 14 - 23 = 4 - 6 = -2
A^-1 = (1/-2) * [ 4  -2 ] = [ -2   1  ]            [ -3  1 ]   [ 1.5 -0.5 ]
Проверка: A * A^-1 = [ 1  0 ]                 [ 0  1 ]
...
Сохраните файлы:
Поместите MatrixCalculatorUI.java, att1.java, att2.java, att3.java в папку src/ml.


Скомпилируйте:cd /path/to/src
javac ml/*.java


Запустите:java ml.MatrixCalculatorUI
